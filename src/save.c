...
static const char *races[] = { "Human", "Half Elf","Elf","Hobbit", "Gnome", "Dwarf", "Half Orc", "Half Troll","Dunadan", "High Elf","Kobold","Maia","Ent","Wraith"};
static const char *classes[] = { "Warrior","Mage","Priest","Rogue","Ranger","Paladin","Illusionist","Druid"};
...
static void wr_extra(void)
{
	...
	wr_s16b(p_ptr->ts_anchor);
	wr_s16b(p_ptr->hero);
	wr_s16b(p_ptr->shero);
	wr_s16b(p_ptr->shield);
	wr_s16b(p_ptr->blessed);
	wr_s16b(p_ptr->tim_invis);
	wr_s16b(p_ptr->tim_pl_invis);
	wr_s16b(p_ptr->tim_ghostly);
	...
	wr_s16b(p_ptr->tim_sus_dex);
	wr_s16b(p_ptr->tim_sus_con);
	wr_s16b(p_ptr->tim_sus_chr);
	...
	wr_s16b(p_ptr->oppose_nexus);
	wr_s16b(p_ptr->oppose_nethr);
...
static void wr_dungeon(void)
{
	...
	for (y = 0; y < DUNGEON_HGT; y++)
	{
		...
		{
			/* Extract a byte */
			tmp16s = cave_special[y][x];

			/* If the run is broken, or too full, flush it */
			if ((tmp16s != prev_char2) || (count == MAX_UCHAR))
			{
				wr_byte((byte)count);
